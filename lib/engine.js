...
function computeHallOfFame(leagues, leagueStatsById, timelineByPlayer) {
  ...
  const records = {
    ...
    grandEcart: null, // plus gros écart de points entre le 1er et le dernier
    peloton: null, // plus petit écart de points entre le 1er et le dernier
  };
  ...
  for (const league of orderedLeagues) {
    ...
    if (stats.firstPlayerId !== null && stats.lastPlayerId !== null) {
      const gapFirstLast = stats.firstPlayerPoints - stats.lastPlayerPoints;
      maybeUpdate('grandEcart', gapFirstLast, stats.firstPlayerId, league.id, (a, b) => a > b);
      maybeUpdate('peloton', gapFirstLast, stats.firstPlayerId, league.id, (a, b) => a < b);
    }
  }
