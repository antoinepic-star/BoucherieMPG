...
/** Détenteurs des 5 records Hall of Fame, en rejouant l'historique dans l'ordre chronologique. */
function computeHallOfFame(leagues, leagueStatsById) {
  ...
  function maybeUpdate(key, candidateValue, holderId, leagueId, better) {
    ...
    if (current === null || better(candidateValue, current.value)) {
      records[key] = {
        ...
        sinceLeagueId: leagueId,
        previousHolderId: current && current.holderId !== holderId ? current.holderId : null,
      };
    }
  }
...
function computeAll({ players, leagues, results }) {
  ...
  const hallOfFame = computeHallOfFame(leagues, leagueStatsById);
