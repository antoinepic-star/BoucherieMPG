...
async function initDb() {
  ...
  const ongoingLeagueColumns = await tursoAll('PRAGMA table_info(ongoing_league)');
  const ongoingLeagueColumnNames = ongoingLeagueColumns.map((c) => c.name);
  if (!ongoingLeagueColumnNames.includes('photo_url')) {
    await tursoRun('ALTER TABLE ongoing_league ADD COLUMN photo_url TEXT');
  }
