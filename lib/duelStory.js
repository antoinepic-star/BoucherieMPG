...
const HOF_VALUE_PHRASE = {
  ...
  dimanche: (v) => `${formatPoints(v)} points seulement pour une victoire`,
  boulet: (v) => `${formatPoints(v)} points malgré une dernière place`,
  phenix: (v) => `${formatPoints(v)} place${v === 1 ? '' : 's'} remontées d'une ligue à la suivante`,
  icare: (v) => `${formatPoints(v)} place${v === 1 ? '' : 's'} perdues d'une ligue à la suivante`,
  grandEcart: (v) => `${formatPoints(v)} points d'écart entre le 1er et le dernier`,
  peloton: (v) => `seulement ${formatPoints(v)} points d'écart entre le 1er et le dernier`,
};
